...
export class OrderService {
    ...
    constructor(
        ...
        trainerClassRepository: Repository<TrainerClass> = AppDataSource.getRepository(TrainerClass),
    ) {
    ...
    async createOrder(orderData: OrderDTO, orderDetailsData: OrderDetailDTO[]): Promise<Order> {
        ...
        await Promise.all(orderDetailsData.map(async (detail) => {
            const gymPackage = detail.gymPackageId
        ? await this.gymPackageRepository.findOne({ where: { id: detail.gymPackageId } })
        : null;
            ...
            const promotion = detail.promotionId
                ...
                : null;
            const trainerClass = detail.trainerClassId
                ? await this.trainerClassRepository.findOne({ where: { id: detail.trainerClassId } })
                : null;
            ...
            const orderDetailPromises = timeSlots.map(async (timeSlot) => {
                const orderDetail = this.orderDetailRepository.create({
                    order: savedOrder,
                    gymPackage: gymPackage || undefined, 
                    trainer,
                    trainerClass: trainerClass || undefined,  // Add trainerClass here
                    price: detail.price,
                    sessionDate: new Date(detail.sessionDate),
                    sessionTime: timeSlot,
                    promotion: promotion || undefined,
                    status: detail.status || 'pending' // Default status if none is provided
                });
        ...
        // Create QR code for all session info
        ...
        qrCodes.push({
            ...
            date: new Date(), // You can adjust this value if needed
            time: 'Multiple Times', // Can be adjusted as well
            qrCode
        });
        ...
        const completedOrder = await this.orderRepository.findOne({
            ...
            relations: ['orderDetails', 'orderDetails.trainer', 'orderDetails.gymPackage', 'orderDetails.promotion', 'orderDetails.trainerClass'] // Include trainerClass in the relations
        });
    ...
    async getOrderDetailsByUserId(userId: number): Promise<OrderDetail[]> {
        ...
        const orderDetails = await this.orderDetailRepository.find({
            ...
            relations: ['order', 'gymPackage', 'trainer', 'promotion', 'trainerClass'] // Thêm các quan hệ cần thiết nếu cần
        });
        ...
        for (const line of lines) {
            ...
            if (match) {
                ...
                const orderDetails = await this.orderDetailRepository.find({
                    ...
                    relations: ['trainer', 'gymPackage', 'trainerClass']
                });
              ...
              // Kiểm tra trainerClass và sessionTime trước khi sử dụng
for (const orderDetail of orderDetails) {
    if (orderDetail.trainerClass && orderDetail.trainerClass.startTime) {
        const classTime = orderDetail.trainerClass.startTime; // Thời gian từ trainerClass
        const scheduledTime = new Date(`${orderDate}T${classTime}:00`);

        if (isNaN(scheduledTime.getTime())) {
            console.error(`Thời gian không hợp lệ cho chi tiết đơn hàng ID ${orderDetail.id}: ${scheduledTime}`);
        ...
        console.error(`Dữ liệu không hợp lệ cho chi tiết đơn hàng ID ${orderDetail.id}: Không tìm thấy trainerClass hoặc sessionTime`);
    }
}
    ...
    // Thêm phương thức lấy chi tiết hóa đơn theo ID hóa đơn
    async getOrderDetailsByOrderId(orderId: number): Promise<OrderDetail[]> {
        // Truy vấn danh sách order details dựa trên orderId
        const orderDetails = await this.orderDetailRepository.find({
            where: { order: { id: orderId } }, // Liên kết với orderId
            relations: ['order', 'gymPackage', 'trainer', 'promotion'], // Các mối quan hệ cần thiết
        });
