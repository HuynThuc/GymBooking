...
import { GoogleUserDTO, UpdatePasswordDTO } from '../dto/register-user.dto';
...
router.put('/change-password/:userId', async (req: Request, res: Response) => {
    const userId = parseInt(req.params.userId);
    const updatePasswordDTO: UpdatePasswordDTO = req.body;

    try {
        const result = await authService.changePassword(userId, updatePasswordDTO);
        res.status(result.status).json({
            status: result.status === 200 ? 'success' : 'error',
            message: result.message,
        });
    } catch (error) {
        // Đảm bảo gửi phản hồi đúng định dạng JSON
        if (error instanceof Error) {
