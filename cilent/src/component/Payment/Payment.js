...
const BookingConfirmation = () => {
  ...
  console.log('ngày', selectedClass)
  ...
  const handleMomoPayment = async () => {
    try {
      const orderDetails = selectedClass ? [{
        gymPackageId: null,
        trainerId: selectedClass.trainer.id,
        trainerClassId: selectedClass.id,
        price: selectedClass.price,
        sessionDate: moment(selectedClass.classDate).format('YYYY-MM-DD'),
        sessionTime: selectedClass.startTime,
        ...
        trainerClassId: null,
        price: finalPrice,
        sessionDate: moment(slot.date).format('YYYY-MM-DD'),
        sessionTime: slot.hour,
        promotionId: appliedCode?.id
      }));

      const requestData = {
        userId: user.id,
        totalAmount: selectedClass ? selectedClass.price : finalPrice, // Ưu tiên lấy giá từ selectedClass nếu có
        status: 'pending',
        orderDetails,
      };
      ...
      toast.error('Đã xảy ra lỗi trong quá trình tạo thanh toán MoMo.');
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    if (!formData.agreeToTerms || !trainerId) {
    ...
    if (!selectedClass && (!selectedSlots || selectedSlots.length === 0)) {
            ...
            <div className="space-y-3">
                  ...
                  <div className="flex space-x-4">
                    <div className="flex-1">
                      ...
                      <span className="font-medium pl-2">
                        {moment(selectedClass.classDate).format('DD/MM/YYYY')}
                      </span>
                    </div>
                    <div className="flex-1">
              ...
              <div className="flex items-center">
                ...
                <span className="font-medium pl-2 text-lg text-blue-600">
                  {selectedClass ? (
                    // Hiển thị giá của selectedClass
                    Number(selectedClass.price).toLocaleString('vi-VN', { style: 'decimal' }) + 'đ'
                  ) : (
